  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {files.length === 0 && (
          <p className="text-[10px] text-text-muted/40 text-center mt-6 px-4">
        ...
        {files.map(([file, items]) => (
          <div key={file}>
            <div className="flex items-center gap-1.5 px-3 py-1">
              ...
              <span className="text-[9px] font-mono text-text-muted/50 truncate flex-1">{file}</span>
              <span className="text-[9px] font-mono text-text-muted shrink-0">{items.length}</span>
            </div>
            {items.slice(0, 50).map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-text-secondary leading-snug line-clamp-2 flex-1">
                    {p.message}
                  </span>
